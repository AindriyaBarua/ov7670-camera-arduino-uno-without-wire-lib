...
void arduinoUnoInut(void) {
  ...
  * This will be on pin 11*/
  DDRB |= (1 << 3);//pin 11 TO SET DIRECTION OF GPIO PIN(INPUT OR OUTPUT), 
  ASSR &= ~(_BV(EXCLK) | _BV(AS2));
  TCCR2A = (1 << COM2A0) | (1 << WGM21) | (1 << WGM20);
  TCCR2B = (1 << WGM22) | (1 << CS20);
  OCR2A = 0;//(F_CPU)/(2*(X+1))
  //DDRB is a bitmap oriented register that controls the Data Direction of each bit on Port B
...
static void captureImg(uint16_t wg, uint16_t hg){
  ...
  StringPgm(PSTR("*RDY*"));//ITS A MACRO THAT STORES IN FLASH, WE HAVE MORE FLASH, ELSE SRAM WILL BE FILLED UP
  ...
  while (y--){
        x = wg;
      //while (!(PIND & 256));//wait for high PINB is a register your code reads to get the state of input bits on Port B
    while (x--){
    ...
    //  while ((PIND & 256));//wait for low PIN-D IS READ ONLY IT CHECKS STATUS OF PIN D
  }
    _delay_ms(100);
}
